# %% [markdown]
# # Air Mass RGB (RGB example using reprojected IR channels)
# This jupyter notebook shows how to make a sub-region plot of air mass RGB composition with reprojected data. This is an example of RGB composition using reprojected IR channels of GOES-16.
...
# %%
C08, LonCen, LatCen = C08_ds.image('CMI', lonlat='center', domain=domain)
...
# %% [markdown]
# Reprojects images of C08, C10, C12 and C13.
...
# %%
del C08, C10, C12, C13, LonCen, LatCen, mask, SwathDef, LonCenCyl, LatCenCyl
...
mask = np.where(np.isnan(C08Cyl)==True, np.nan, 1).astype(np.float16)
...
filename = f"G16_air_mass_RGB_SRCYL_plot_{timestamp}.png"  # Example filename: plot_20230714123456.png
